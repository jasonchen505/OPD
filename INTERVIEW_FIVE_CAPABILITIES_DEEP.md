...
### Q1.1: OPD 到底解决什么问题？为什么不能只用 GRPO？

**问题背景**：

GRPO 是 outcome-level RL，每个 response 只有一个最终 reward（正确/错误）。在数学推理任务上，1.5B 模型早期正确率极低，大部分 batch reward 全 0，policy gradient 信号很弱。

**核心机制**：

OPD 让 student 自己 rollout，然后在每个 token 位置用 teacher 的分布计算 KL reward。这样即使最后答案错了，中间每个 token 都有 teacher 的分布信号。
...
### Q1.2: 为什么用 reverse KL 而不是 forward KL？
...
OPD 用 reverse KL `KL(π_student || π_teacher)`。这意味着：
...
### Q1.4: `token_reward_direct` 为什么不需要 critic？
...
**与 GRPO 的对比**：
- GRPO：需要组内比较，`advantage = (reward - mean) / std`
- OPD：直接用 KL reward，不需要分组

**局限**：
- verl 代码库是 PPO/RL 通用框架，会保留 critic 组件
- 如果用 `token_reward_direct_plus_grpo`，GRPO 部分仍需要组内比较

---

### Q1.5: PPO clipping 在 OPD 下怎么工作？

**问题背景**：

OPD 的 top-K 情况下，advantage 是 3D `(batch, seq_len, K)`，不是普通的 2D。
...
**一句话**：
> OPD 更像把 distillation 嵌进 on-policy RL 框架，用 teacher distribution 替代稀疏 outcome reward。SFT 是 imitation，DPO 是 preference optimization，GRPO 是 outcome RL，OPD 是 distribution alignment。
...
### Q2.3: mean_score 和 best_score 怎么解读？
...
**面试要点**：
> 简历和面试里应该重点讲 mean_score，同时说明 best_score 反映多样采样潜力，不能直接当单次 accuracy。

---

### Q2.4: 你会怎么做更严谨的 ablation？
...
### Q4.2: vLLM `gpu_memory_utilization` 为什么要降到 0.4？
...
### Q4.3: FSDP offload 为什么有用？代价是什么？

**核心机制**：

FSDP 已经把参数、梯度、优化器分片到多卡，但 24GB GPU 仍然紧张。param offload 和 optimizer offload 把部分状态放到 CPU。
...
**项目中的实践**：
> 我的评估就是用 merged HF checkpoint `DeepSeek-R1-Distill-Qwen-1.5B-OPD-final`，vLLM 生成 AIME/AMC 输出。这个链路和生产推理相似。
...
### Q4.7: flash-attn fallback 问题是什么？

**问题背景**：

远程环境 CUDA 13.1 和 flash-attn 预编译 wheel 不兼容。

**我做了什么**：
- 加了一个纯 PyTorch fallback，让 FSDP + sdpa 路径跑通
...
**风险**：
- 某个 GPU OOM 或 tokenizer error 可能导致不完整评测文件
- 评分脚本仍会计算结果，指标在不完整样本上被错误解释
...
> 业务价值不是 OPD 本身，而是用强 teacher 提升小 student，降低线上成本或提升任务成功率。它适合高频、长推理、teacher 调用贵、又有明确业务 prompt 分布的场景。
>
